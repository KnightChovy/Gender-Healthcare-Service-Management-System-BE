...
const paymentSession = async (user_id, priceVND, appointment_id) => {
  console.log("priceVND", priceVND);

  if (!priceVND || typeof priceVND !== "number") {
    throw new Error("Giá trị price không hợp lệ");
  }

  const exchangeRate = 25000; // 1 USD = 25,000 VND (ví dụ, có thể cập nhật theo thực tế)
  const priceUSD = priceVND / exchangeRate;
  const unitAmount = Math.round(priceUSD * 100); // Stripe dùng đơn vị cent
